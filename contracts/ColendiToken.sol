...
contract ColendiToken is ERC20 {
    ...
    function metaTransfer(bytes calldata signature, address to, uint256 value, uint256 nonce, uint256 reward, uint256 gasLimit)
    external returns (bool) {
        uint256 startGas = gasleft();
        bytes32 metaHash = metaTransferHash(to,value,nonce,reward, gasLimit);
        address signer = checkSignatureAndPayReward(metaHash, signature, nonce, reward, startGas, gasLimit);
        _transfer(signer, to, value);
        return true;
    }
    function metaTransferHash(address to, uint256 value, uint256 nonce, uint256 reward, uint256 gasLimit) public view returns(bytes32){
        return keccak256(abi.encodePacked(address(this),"metaTransfer", to, value, nonce, reward, gasLimit)).toEthSignedMessageHash();
    }

    function metaApprove(bytes calldata signature, address spender, uint256 value, uint256 nonce, uint256 reward, uint256 gasLimit)
    external returns (bool) {
        uint256 startGas = gasleft();
        bytes32 metaHash = metaApproveHash(spender,value,nonce,reward, gasLimit);
        address signer = checkSignatureAndPayReward(metaHash, signature, nonce, reward, startGas, gasLimit);
        _approve(signer, spender, value);
        return true;

    }
    function metaApproveHash(address spender, uint256 value, uint256 nonce, uint256 reward, uint256 gasLimit) public view returns(bytes32){
        return keccak256(abi.encodePacked(address(this),"metaApprove", spender, value, nonce, reward, gasLimit)).toEthSignedMessageHash();
    }

    function metaTransferFrom(
        bytes calldata signature,address sender,address recipient,uint256 value,uint256 nonce,uint256 reward, uint256 gasLimit
    ) external returns (bool)
    {
        uint256 startGas = gasleft();
        bytes32 metaHash = metaTransferFromHash(sender, recipient,value,nonce,reward, gasLimit);
        address signer = checkSignatureAndPayReward(metaHash, signature, nonce, reward,startGas, gasLimit);
    ...
    function metaTransferFromHash(address sender, address recipient, uint256 value, uint256 nonce, uint256 reward, uint256 gasLimit)
    public view returns(bytes32){
        return keccak256(
            abi.encodePacked(address(this),"metaTransferFrom", sender, recipient, value, nonce, reward, gasLimit)
